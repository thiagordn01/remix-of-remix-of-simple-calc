...
export function useSystemSettings() {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.new && (payload.new as any).value) {
            setMaintenanceMode((payload.new as any).value as MaintenanceSettings);
          }
        },
      )
      .subscribe();
  ...
  const fetchMaintenanceStatus = async () => {
    try {
      ...
      if (data && data.value) {
        // CORREÇÃO AQUI: Usamos 'unknown' como ponte para converter Json em MaintenanceSettings
        setMaintenanceMode(data.value as unknown as MaintenanceSettings);
      }
    } catch (error) {
