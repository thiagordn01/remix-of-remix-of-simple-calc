...
export class EnhancedGeminiService {
    ...
    try {
      ...
      data.forEach(({ apiId, blockedUntil, reason }) => {
        if (blockedUntil > now) {
          ...
          console.log(`🔒 API ${apiId} carregada em quarentena até ${new Date(blockedUntil).toISOString()} - ${reason}`);
        }
      });
    } catch (error) {
  ...
  private createApiError(message: string, status?: number, retryable = true, quotaInfo?: ApiError['quotaInfo']): ApiError {
  ...
  private analyzeApiResponse(data: any): {
    ...
    const hasContent = !!parts && parts.length > 0 && 
                       parts.some((p: any) => p.text && p.text.trim().length > 0);
  ...
  private makePromptSafer(prompt: string): string {
    ...
    const sensitivePatterns = [
      /\b(replace|substituir|eliminar|destruir)\b/gi,
      /\b(controversial|polêmico)\b/gi
    ];
  ...
  private async makeApiCallWithTimeout(
    ...
    try {
      ...
      throw this.createApiError(
        `Erro de conexão com API ${apiKey.name}: ${error.message}`,
        undefined,
        true
      );
    }
  ...
  private shouldBlockKey(error: ApiError, failureCount: number): {
    ...
    if (errorMessage.includes('safety') ||
        errorMessage.includes('segurança') ||
    ...
    if (errorMessage.includes('nenhum conteúdo') ||
  ...
  private recordApiSuccess(apiKey: GeminiApiKey) {
    ...
    const rpd = (this.apiRequestsPerDay.get(apiKey.id) || [])
      .filter(t => t > oneDayAgo); // ✅ BUGFIX: Remover timestamps > 24h
    rpd.push(now);
    ...
    console.log(`✅ API ${apiKey.name} - Sucesso registrado (RPM atual: ${rpm.length}, RPD atual: ${rpdFiltered.length})`);
  }
  ...
  private canUseApi(apiKey: GeminiApiKey): boolean {
    ...
    if (lastRequestTime) {
      ...
      if (timeSinceLastRequest < this.MIN_TIME_BETWEEN_REQUESTS) {
        ...
        console.log(`⏱️ API ${apiKey.name} precisa aguardar ${remainingSeconds}s (última req há ${Math.round(timeSinceLastRequest/1000)}s)`);
        return false;
      }
    }
    ...
    const oneMinuteAgo = now - 60000;
    const rpmTimestamps = (this.apiRequestsPerMinute.get(apiKey.id) || [])
      .filter(t => t > oneMinuteAgo);
    ...
    if (rpmTimestamps.length > 0) {
      ...
      console.log(`📊 [${apiKey.name}] RPM Check: ${rpmTimestamps.length}/${limits.rpm} requisições no último minuto (mais antiga há ${ageOfOldest}s)`);
    }
    ...
    if (rpmTimestamps.length >= limits.rpm) {
      ...
      console.warn(`⏸️ API ${apiKey.name} atingiu ${limits.rpm} RPM (limite do modelo ${apiKey.model}). Disponível em ${waitTime}s`);
      return false;
    }

    // 3. Verificar RPD (últimas 24h)
    const oneDayAgo = now - 86400000;
    const rpdTimestamps = (this.apiRequestsPerDay.get(apiKey.id) || [])
      .filter(t => t > oneDayAgo);
    ...
    if (rpdTimestamps.length >= limits.rpd) {
      console.warn(`🛑 API ${apiKey.name} atingiu ${limits.rpd} RPD (limite do modelo ${apiKey.model}). Exaurida pelo resto do dia.`);
      return false;
    }
  ...
  private lockApi(apiKey: GeminiApiKey) {
    ...
    const oneMinuteAgo = now - 60000;
    const rpm = (this.apiRequestsPerMinute.get(apiKey.id) || [])
      .filter(t => t > oneMinuteAgo); // ✅ BUGFIX: Remover timestamps > 60s
    rpm.push(now);
  ...
  private enhancePromptForFidelity(
  ...
  async generateWithFidelity(
    ...
    context: GenerationContext = {},
    options: GenerationOptions = {}
  ): Promise<{ content: string; usedApiId: string }> { // ✅ MUDANÇA: retornar objeto com API usada
    const {
    ...
    while (totalAttempts < MAX_TOTAL_ATTEMPTS) {
      ...
      if (availableApisForThisRound.length === 0) {
        ...
        if (apisInCooldown.length > 0) {
          ...
          const nextAvailable = Math.min(
            ...apisInCooldown.map(api => this.keyCooldownUntil.get(api.id) || 0)
          );
          const waitTime = Math.ceil((nextAvailable - Date.now()) / 1000);

          if (waitTime > 0 && waitTime <= 60) { // Só esperar se for menos de 1 minuto
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (elapsedTime > MAX_TOTAL_TIME_MS) {
          ...
          throw new Error(`Timeout: todas as APIs falharam após ${Math.round(elapsedTime/1000)}s (${totalAttempts} tentativas)`);
        }
        ...
        try {
          ...
          const result = await this.makeApiCallWithTimeout(enhancedPrompt, api, {
            ...options,
            onProgress
          }, attempt, context);
          ...
          if (blockInfo.shouldBlock) {
            ...
            const shouldRemoveRpmTimestamp = !apiError.status || // Sem status HTTP = erro local
            ...
            if (shouldRemoveRpmTimestamp) {
              ...
              if (rpmTimestamps.length > 0) {
                ...
                console.log(`♻️ [${api.name}] Timestamp RPM removido (erro ${apiError.status || 'local'}, não consumiu quota)`);
              }
            } else {
              console.log(`📊 [${api.name}] Timestamp RPM mantido (requisição consumiu quota, status: ${apiError.status})`);
            }
          ...
          if (apiError.code === 'RATE_LIMIT') {
            ...
            const currentRpm = (this.apiRequestsPerMinute.get(api.id) || [])
              .filter(t => now - t < 60000).length;
            const currentRpd = (this.apiRequestsPerDay.get(api.id) || [])
              .filter(t => now - t < 86400000).length;

            // Log detalhado para debug
            console.log(`📊 [${api.name}] Rate Limit 429 - RPM: ${currentRpm}/${limits.rpm}, RPD: ${currentRpd}/${limits.rpd}`);
            ...
            if (quotaId.includes('PerMinute') && !quotaId.toLowerCase().includes('token')) {
              ...
              onProgress?.(`⏸️ API ${api.name} atingiu RPM. Cooldown ${Math.ceil(cooldownMs/1000)}s (Google: ${retryDelay || '?'}s)`);
            ...
            if (quotaId.toLowerCase().includes('token') || (currentRpm < limits.rpm && currentRpd < limits.rpd)) {
              ...
              this.keyCooldownUntil.set(api.id, now + cooldownMs);
              onProgress?.(`⚠️ API ${api.name}: Limite TPM. Cooldown ${Math.ceil(cooldownMs/1000)}s (Google: ${retryDelay || '?'}s)`);
              break;
            }
          ...
          if (this.apiInUse.get(api.id)) {
            ...
            const shouldClearTimer = lastError && (
              !lastError.status ||
  ...
  private addCulturalContext(prompt: string, context: GenerationContext): string {
    ...
    culturalPrompt += `\n- MANTENHA tom e estilo apropriados para a região`;
