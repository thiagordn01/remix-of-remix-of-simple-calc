...
import { Play, Copy, Download, Bot, Settings, FileText, Sparkles, ArrowRight, Zap } from 'lucide-react';
import { ScriptGenerationRequest, AIProvider } from '@/types/scripts';
...
export const ScriptGenerator = ({ onScriptGenerated }: ScriptGeneratorProps) => {
  ...
  // Verificar disponibilidade do Puter.js
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    if (effectiveProvider === 'deepseek') {
      ...
      // Garantir que o usuario esta autenticado
      const authenticated = await puterDeepseekService.ensureAuthenticated();
      if (!authenticated) {
        toast({
          title: "Login necessario",
          description: "Faca login no Puter na aba 'APIs' > 'DeepSeek' para usar gratuitamente.",
          variant: "destructive"
        });
        return;
      }
        ...
        <TabsContent value="generate" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Selecao de Provedor de IA */}
              <div className="p-4 border rounded-lg space-y-3">
                <div className="flex items-center gap-2">
                  <Zap className="w-4 h-4 text-primary" />
                  <Label className="font-medium">Provedor de IA</Label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedProvider === 'gemini' ? (
                    <>
                      ...
                      <Badge variant={activeGeminiKeys.length > 0 ? "default" : "destructive"}>
                        {activeGeminiKeys.length} ativa{activeGeminiKeys.length !== 1 ? 's' : ''}
                      </Badge>
                    </>
                  ) : (
                    <>
                      <span className="text-sm">Puter.js:</span>
                      <Badge variant={isPuterReady ? "default" : "secondary"}>
                        {isPuterReady ? 'Conectado' : 'Faca login'}
                      </Badge>
                    </>
                  )}
                </div>
