...
const DEFAULT_AGENTS: Agent[] = [
  {
    ...
    durationMinutes: 10,
    premisePrompt:
      "Crie uma premissa cinematográfica baseada neste título. Defina o protagonista, o cenário único (world-building), o incidente incitante e os riscos emocionais. Faça parecer a sinopse de um filme premiado.",
  ...
  },
  {
    ...
    scriptPrompt:
      "Write a high-retention script text ONLY. No visual cues, no [brackets]. Use slang, keep sentences short. Direct address to camera. The text must be ready to read out loud immediately.",
  },
];
...
function App() {
    ...
    try {
      for (let i = 0; i < titles.length; i++) {
        ...
        try {
          ...
          setBatchItems((prev) =>
            prev.map((item) => (item.id === batchId ? { ...item, status: "completed", content: finalContent } : item)),
          );
        } catch (singleErr: any) {
          console.error(singleErr);
          setBatchItems((prev) =>
            prev.map((item) =>
              item.id === batchId ? { ...item, status: "error", errorMsg: singleErr.message } : item,
            ),
          );
        }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-10 px-4 md:px-8 max-w-7xl mx-auto relative">
      ...
      <input type="file" ref={fileInputRef} onChange={importAgents} accept=".json" className="hidden" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#1a1a1a] border border-gray-700 rounded-2xl w-full max-w-3xl p-6 shadow-2xl relative flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
              {history.length === 0 ? (
                <div className="text-center text-gray-500 py-10">Nenhum roteiro salvo ainda.</div>
              ) : (
                history.map((item) => (
                  <div
                    key={item.id}
                    className="bg-gray-800/50 hover:bg-gray-800 border border-gray-700 rounded-xl p-4 transition-all group flex justify-between items-center"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingAgent({ ...editingAgent, durationMinutes: parseInt(e.target.value) || 1 })
                    }
                  />
        ...
        <div className="flex items-center gap-3 bg-gray-900/80 p-2 pr-4 rounded-xl border border-gray-800">
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              <option value="new" className="bg-gray-900 text-red-400 font-bold">
                + Criar Novo Agente
              </option>
            </select>
          </div>
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="glass-panel p-6 rounded-2xl space-y-6 shadow-xl">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-red-400 uppercase tracking-wider flex justify-between">
                ...
                <span className="text-[10px] text-gray-500 normal-case font-normal">
                  (Um por linha para gerar em lote)
                </span>
              </label>
              <textarea
                name="title"
                ...
                rows={3}
                placeholder={
                  "Digite um título por linha para gerar vários roteiros de uma vez...\nEx: A História do Café\nO Mistério do Oceano"
                }
            ...
            {error && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-200 text-sm">{error}</div>
            )}
              ...
              {batchStatus ? (
                <>
                  ...
                  <div className="flex flex-col items-start leading-tight">
                    <span>
                      Gerando Lote ({batchStatus.current}/{batchStatus.total})
                    </span>
                    <span className="text-xs font-normal opacity-80">
                      {stepStatus === "generating-premise"
                        ? "Criando Premissa..."
                        : `Escrevendo Parte ${generationProgress?.current || 1}/${generationProgress?.total || "?"}`}
                    </span>
                  </div>
                </>
              ) : stepStatus === "generating-premise" ? (
                ...
                <>
                  <LoaderIcon /> Escrevendo Roteiro{" "}
                  {generationProgress ? `(Parte ${generationProgress.current}/${generationProgress.total})` : ""}...
                </>
              ) : (
                <>
            ...
            <div className="glass-panel p-4 rounded-2xl shadow-xl max-h-[300px] overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center mb-3 sticky top-0 bg-[#1e1e1ec0] backdrop-blur-md pb-2 z-10 border-b border-gray-700">
                <h3 className="text-white font-bold flex items-center gap-2">
                  <ListIcon /> Sessão Atual ({batchItems.filter((i) => i.status === "completed").length}/
                  {batchItems.length})
                </h3>
              ...
              <div className="space-y-2">
                {batchItems.map((item) => (
                  <div
                    key={item.id}
                    className={`p-3 rounded-lg border flex items-center justify-between gap-3 ${item.status === "generating" ? "bg-red-900/10 border-red-500/30" : "bg-gray-800/50 border-gray-700"}`}
                  >
                  ...
                  ? cleanArtifacts(output)
                      ...
                      .map((line, i) => {
                        ...
                        if (viewMode === "clean") {
                          ...
                          if (
                            line.startsWith("## 🎬 PREMISSA") ||
                            (scriptStart > -1 && output.indexOf(line) < scriptStart)
                          ) {
                            return null;
                          }
                        ...
                        if (trimmed.startsWith("## ")) {
                          return (
                            <h3
                              key={i}
                              className="text-2xl font-bold text-white mt-10 mb-6 border-b border-gray-700 pb-2"
                            >
                              {trimmed.replace("## ", "")}
                            </h3>
                          );
                        ...
                        return (
                          <p key={i} className="text-gray-200 mb-4 leading-7">
                            {trimmed}
                          </p>
                        );
                      })
