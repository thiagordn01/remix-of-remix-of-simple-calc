...
// Supabase Edge Function: Proxy to Cloudflare Worker 2 (sparkling-band-8e85)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const url = new URL("https://sparkling-band-8e85.ouroferrero008.workers.dev/");
    url.searchParams.set("input", input);
    url.searchParams.set("prompt", prompt);
    url.searchParams.set("voice", voice);
    url.searchParams.set("generation", gen);

    console.log("Proxying to worker2:", url.toString());
    ...
    console.error("worker2-proxy error:", err?.message || err);
