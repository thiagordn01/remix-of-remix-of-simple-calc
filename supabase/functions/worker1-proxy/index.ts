...
// Supabase Edge Function: Proxy to Cloudflare Worker 1 (ancient-leaf-8595)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const url = new URL("https://ancient-leaf-8595.ouroferrero008.workers.dev/");
    url.searchParams.set("input", input);
    url.searchParams.set("prompt", prompt);
    url.searchParams.set("voice", voice);
    url.searchParams.set("generation", gen);

    console.log("Proxying to worker1:", url.toString());
    ...
    console.error("worker1-proxy error:", err?.message || err);
