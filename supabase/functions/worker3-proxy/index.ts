...
// Supabase Edge Function: Proxy to Cloudflare Worker 3 (purple-leaf-a063)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const url = new URL("https://purple-leaf-a063.ouroferrero008.workers.dev/");
    url.searchParams.set("input", input);
    url.searchParams.set("prompt", prompt);
    url.searchParams.set("voice", voice);
    url.searchParams.set("generation", gen);

    console.log("Proxying to worker3:", url.toString());
    ...
    console.error("worker3-proxy error:", err?.message || err);
