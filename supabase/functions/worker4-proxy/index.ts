...
// Supabase Edge Function: Proxy to Cloudflare Worker 4 (tiny-hall-c75a)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const url = new URL("https://tiny-hall-c75a.ouroferrero008.workers.dev/");
    url.searchParams.set("input", input);
    url.searchParams.set("prompt", prompt);
    url.searchParams.set("voice", voice);
    url.searchParams.set("generation", gen);

    console.log("Proxying to worker4:", url.toString());
    ...
    console.error("worker4-proxy error:", err?.message || err);
